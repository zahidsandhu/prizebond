...
jQuery(document).ready(function(){
    ...
    $(".Prizebond-toggle").click(function(){
       var elem = $(this);
	...
	$.screen = function(c, p) {
		...
		switch (c) {
			case 'lock':
                ...
                if (!$.browser.msie || ($.browser.msie == true && $.browser.version=='10.0')) {
...
function updateInfo(result) {
	...
	if (typeof result.disabledPaymentMethods != 'undefined') {
		var $methods = $('#saved-payment-methods,#saved-payment-methods-tiny').find(':radio').prop({disabled: false});
		for (var index in result.disabledPaymentMethods) {
			$methods.filter('[value='+result.disabledPaymentMethods[index]+']').prop({checked: false, disabled: true});
		}
	}
...
$(function () {
	...
	var accountsPopup = new function() {
		...
		this.init = function() {
			...
			$('#edit_billing_details,#change_password,#acc_exist,#change_login').click(function() {
				var email;
				var emailValue ='';
		...
		function showPriceHint(hint,currentElement,hintType) {
			...
			if (hintType=='price') {
				...
				hintValue='<p>'+currentElement.data('alt')+'</p>';
                        ...
                        else if (hintType=='offer') {
				var elemId=currentElement.data('alt');
				var priceTitle=document.getElementById(elemId);
				hintValue=priceTitle.innerHTML;
			}
		...
		$("a.productType.altTitle").each(function() {
			...
			currentElement.data('alt',currentElement.prop('title')).prop('title','');
				...
				hintMouseFollow(event,priceHint,$(window).height(),$(window).width(),hint.height,hint.width,$(window).scrollTop());
                        ...
                        $("label.offer-title.altTitle").each(function() {
                var currentElement=$(this);
                currentElement.data('alt',currentElement.prop('title')).prop('title','');
        }).hover(function() {
                        if ($(this).data('alt')!='') {
                                showPriceHint(priceHint,$(this),'offer');
                                hint.height=priceHint.clientHeight;
                                hint.width=priceHint.clientWidth;
                        }
                }, function() {
...
$(document).ready(function() {
	...
	)
	{
		...
		$('#gui-delivery-cart-submit').click(function() {
			$('#saved-payment-methods-tiny :input').attr('disabled', true);
...
/*preview2 for prestashop*/
		
$(document).ready(function()
{
    if (1 == $('#PreviewPage2').length)
	{
		/* Инициализация начального состояния чекбокса цены */
		var price_options_form = $('#priceVariantsForm');
		var regular_price_input = $('#regular_price');
		var regular_install_price_input = $('#regular_install_price');
		var regular2_price_input = $('#regular2_price');
		...
		/* Всплывающие подсказки над ценами офферов */ 
		initDelFullPrice();
		$('del.fullPrice').live('mouseenter', function() {
			showDiscount($(this).data('id'));
		}).live('mousemove', function(event) {
	...
	$.widget('custom.previewCalcPrice',
	{
		_create: function()
		{
			this.calcPrice();
			var self = this;
			$('#template-preview-page li input').live('click', function()
			{
				self.calcPrice();
			});
		},
		...
		{
			var sum = 0;
			sum += parseInt(this.element.parents('.price_block').find('.ui-product-preview-price:first').html());
			$('#template-preview-page li input:checked').each(function()
			{
				var self = $(this);
				if ('true' == self.attr('bs'))
				{
					var oid = self.next().find('input[name^="offer_bundle_select["]:first').val();
					self.attr('price', self.next().find('li[val="' + oid + '"]:first').attr('price'));
				}
				sum += parseInt(self.attr('price'));
			});
...
function showBannerRedesign() {
    if (_gaq)
    {
        _gaq.push(['b._trackEvent', 'banner-redesign', 'show']);
    }
    var myDate = new Date();
    myDate.setMonth(myDate.getMonth() + 1);
    document.cookie = 'betabanner=1; expires='+myDate.toGMTString()+'; path=/; domain=' + window.location.hostname.split('.').slice(-2).join('.');
    $("body").append('<div id="banner-redesign"><a href="#" class="close_banner_popup" onclick="$(\'#banner-redesign\').popup(\'close\'); return false;"></a><a id="banner-redesignid" href="http://beta.templatemonster.com/" onclick="_gaq.push([\'b._trackEvent\', \'banner-redesign\', \'click\']);return true;"><img src="http://i.tmimgcdn.com/library/redesign-big.png" alt="New Tempaltemonster"></a></div>');
    $('#banner-redesignid').attr('href','http://beta.templatemonster.com' + window.location.pathname+window.location.search);
    $('#banner-redesign').popup('open');
}
