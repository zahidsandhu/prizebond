...
<form name="form1" method="post" action="<?php echo "$url"; ?>">
  <table width="100%" cellpadding="10" cellspacing="0">
    ...
    <tr>
	  ...
	  if (isset($_POST["Submit"])){
			...
			if (strip_tags($_POST["posting"])==""){
				echo "<div style='background-color:#fee4e0;color:#900;line-height:22px;padding:10px;margin:5px;border:2px solid #900'><p style='color:#c00'>You can not post empty.</p></div>";
			}else{
				$dat=time();
				if ($_POST["Submit"]=="Submit"){
				mysqli_query($con,"insert into posting (uid,posting, dat, page, level) values ('".$_SESSION["uid"]."','$posting','$dat','".$_GET["post"]."','1')");
				}else{
					mysqli_query($con,"update posting set posting='$posting', dat='$dat', level='1' where dat='".$_GET["edit"]."'");
				}
				if ($_GET["post"]=="homepage"){
					mysqli_query($con,"update users set hm='$dat' where uid='".$_SESSION["uid"]."' ");
				}
				header("location:?s=1");
			}
	  }
    ...
    <tr>
      <td align="center"><input type="submit" name="Submit" class="btn btn-primary mr-2" value="<?php echo $button; ?>" class="submit" <?php echo $ds; ?> /></td>
    </tr>
  </table>
</form>
