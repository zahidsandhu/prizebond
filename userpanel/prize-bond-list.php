...
$select_wallet=mysqli_query($con,"SELECT wallet.*,list.dat 
FROM wallet
LEFT JOIN list ON list.draw = wallet.price
WHERE wallet.uid= '".$_SESSION['uid']."' && wallet.price='".$_GET['bond']."'  && (list.numbers like '%' + wallet.bond_number + '%' or list.list like '%' + wallet.bond_number + '%') && list.draw like '%' + wallet.price + '%' ");
}
// print_r($select_wallet);exit();
if(isset($_GET['del'])){
 				...
 				<div class="card">
 					<div class="card-body">
 						...
 						<div class="table-responsive">
 						   ...
 						   $sql = "SELECT wallet.*,list.dat 
                                        FROM wallet
                                        LEFT JOIN list ON list.draw = wallet.price
                                        WHERE wallet.uid= '".$_SESSION['uid']."' && wallet.price='".$_GET['bond']."'  && (list.numbers like '%' + wallet.bond_number + '%' or list.list like '%' + wallet.bond_number + '%') && list.draw like '%' + wallet.price + '%' ";
                                        echo($sql);
                                        $s = mysqli_fetch_array($sql);echo "<pre>";print_r($s);exit();
 						    if(isset($_GET['bond'])){
                                $select_wallet2=mysqli_query($con," SELECT *  FROM wallet WHERE wallet.uid= '".$_SESSION['uid']."' && wallet.price='".$_GET['bond']."'
                                ");
                            }
